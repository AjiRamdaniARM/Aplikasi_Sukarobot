<dialog id="modalExportPDF">
    <h2 class="poppins-bold">Super Filter</h2>
    <form method="POST" action="{{ route('admin.export.laporan.trainer')}}" id="filterDataLaporan">
        @csrf
        <div class="voucher-code-container-admin">
            <h6 class="voucher-code-title-admin">Select Trainer</h6>
            <select name="trainerID" class="voucher-input px-4" required>
        ...
        <br>
    <div>
        <h6 class="text-black px-4">Select Month and Year</h6>
        <input type="month" name="month_year" class="voucher-input px-4 border" required>
    </div>
        <button type="submit" id="submitBtn" class="voucher-button-admin">Export Now</button>
    </form>
    <button onclick="window.modalExportPDF.close();" aria-label="close" class="x">❌</button>
</dialog>
