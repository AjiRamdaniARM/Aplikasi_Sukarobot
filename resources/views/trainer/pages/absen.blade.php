    ...
    <div class="container">
        ...
        <div class="banner">
            ...
            <div data-aos="fade-up" class="card-one lg:py-10 md:py-10 py-6 grid grid-flow-dense lg:grid-cols-2 gap-4 lg:grid-rows-2 grid-cols-1 grid-rows-1">
                <div class="detail-inform w-full bg-white rounded-lg border border-[#000000] p-5 relative">
                    <div class="mini-alert bg-[#EEAA17FF] text-[15px] p-1 border border-[#000000] text-[#000000] poppins-regular absolute top-3 right-3 w-20 rounded-[24px] text-center">
                        {{$getScheduleTrainer->dj_akhir}} Jam
                    </div>
                    <h1 class="poppins-bold lg:text-2xl md:text-2xl">Informasi Jadwal</h1>
                    <p class="poppins-regular "> {{ \Carbon\Carbon::parse($getScheduleTrainer->tanggal_jd)->translatedFormat('d F Y') }},  {{ date('H:i', strtotime($getScheduleTrainer->jm_awal)) }} - {{ date('H:i', strtotime($getScheduleTrainer->jm_akhir)) }}</p>
                    ...
                    <p class="poppins-regular">{{$getScheduleTrainer->levels}}, {{$getScheduleTrainer->program}} , {{$getScheduleTrainer->nama_alat}} </p>
                </div>
                {{-- === component siswa === --}}
                <div class="tab border rounded-lg border-[#000000]">
                    <div class="text-head px-5 mt-3 flex justify-between items-center">
                        ...
                        @if ($getScheduleTrainer->absensi_anak == 'tutup')
                            <button class="bg-[#FF0000FF] text-white border border-[#000000] rounded-[24px] px-3 py-1 hover:scale-105 transition-all bg-[#2E2E4BFF]">Sudah Absen</button>
                        @else
                            <button id="absensi" onclick="window.location.href='{{ url('/absensiswa/' . $getScheduleTrainer->id_schedules) }}   '" class="bg-[#1A1A34] border border-[#000000] text-white rounded-[24px] px-3 py-1 hover:scale-105 transition-all bg-[#2E2E4BFF]">Absensi Siswa</button>
                        @endif
                        <script>
                ...
                {{-- === end component siswa === --}}
                <div class="detail-inform w-full bg-[#FED2D9] rounded-lg border border-[#000000] p-5 relative">
                    <h1 class="poppins-bold lg:text-2xl text-[#75081AFF]">Unggah Dokumentasi ke Google Drive auto atau manual</h1>
                    <button id="unggah" class="bg-[#FB9595FF] border border-[#000000] hover:scale-105 transtion-all hover:bg-[#F86B6BFF]  text-[#75081AFF] px-5 py-2 text-center mt-4 rounded-[24px]">Dokumentasi</button>
                    <script>
                ...
                <div class="content-form">
                    <div class="input-laporan lg:flex-row md:flex-row flex flex-col  justify-between items-center space-y-4 md:space-y-0 md:space-x-4">
                        ...
                        <div class="select-1 w-full md:w-1/2">
                            <select name="materi" id="materi1" class="w-full p-2 border border-[#000000] rounded-md focus:ring focus:ring-indigo-300 focus:border-indigo-500" required>
                        ...
                        <div class="select-2 w-full md:w-1/2">
                            <select name="id_ttd" id="materi2" class="w-full p-2 border border-[#000000] rounded-md focus:ring focus:ring-indigo-300 focus:border-indigo-500" required>
                    ...
                    <div class="box-area py-5">
                        <textarea id="catatan" name="catatan" class="w-full  p-4 border border-[#000000] rounded-md " id="laporan" placeholder="Tulis laporan di sini... (Respone anak , Kendala yang dihadapi dan lainnya)" required></textarea>
                    </div>
                </div>
                {{-- === button absen === --}}
