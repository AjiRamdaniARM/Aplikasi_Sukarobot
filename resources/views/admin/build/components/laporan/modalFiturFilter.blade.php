...
<dialog id="modalFilter">
    ...
    <form method="GET" action="{{ route('laporan.custom') }}" id="filterDataLaporan">
        ...
        <br>
        <div>
            <h6 class="text-black px-4">end date of teaching
            </h6>
            <input type="date" name="end_date" class="voucher-input px-4 border" required>
        </div>

        <button type="submit" id="submitBtn" class="voucher-button-admin">Filter</button>
    </form>
    <button onclick="window.modalFilter.close();" aria-label="close" class="x">❌</button>
</dialog>
